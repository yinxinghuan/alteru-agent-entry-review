...
`v1.0.2` 针对 LottieFiles 与原生渲染差异做了完整兼容：U 使用纯矢量路径；原生 JSON 自带平台黑色底层，避免白色 U 在白色预览器中消失；双眼拆成两个独立的上层 Shape Layer，避免被粉色脸遮住；气泡向角色靠近 8px。前端应替换旧 JSON 文件本身，不要继续使用已缓存的 `v1.0.0 / v1.0.1` 内容。
